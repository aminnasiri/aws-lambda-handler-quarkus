...
- [The demo application](##The-demo-application)
    ...
    - [Deploy the example application to AWS Lambda](###Deploy-the-example-application-to-AWS-Lambda)
    - [Watching performance of the demo application on AWS Lambda + JVM platform](###Watching-performance-of-the-demo-application-on-AWS-Lambda+JVM platform)
...
simple AWS Lambda function. This simple function will accept a fruit name in a JSON format(input) and will return a type of the fruit.
...
Quarkus has very nice guideline to use it and AWS Lambda project is not exception, please follow this one on [Quarkus website](https://quarkus.io/guides/amazon-lambda). 
In a nutshell, we can create it with a Maven command
...
It will generate an application with using AWS Java SDK. The Quarkus framework has extensions for DynamoDB, S3, SNS and SQS and more, and I would like to suggest use AWS Java SDK V2.
So the pom file of project needs to have some modification, and please follow [this guideline](https://quarkus.io/guides/amazon-lambda#aws-sdk-v2) to update it.
...
One of main benefits of Quarkus having Dependency Injection like a normal Java application.
...
`DynamoDbClient` is a class from AWS Java SDK.v2, and Quarkus will build it and make it available in its Dependency Injection ecosystem.
The [FruitService](src/main/java/com/thinksy/service/FruitService.java) is inherited from an abstract class [AbstractService](src/main/java/com/thinksy/service/AbstractService.java) and this abstract class will provide basic objects of `DynamoDbClient` needs, e.g. `ScanRequest`, `PutItemRequest`, etc.

+ **point**: Quarkus makes much more benefits on using AWS Lambda platform and I will describe them in next articles e.g. having multiple rest endpoints in a one AWS Lambda and match it and Api-Gateway.

### Deploy the demo application to AWS Lambda

It's deployment time on AWS, and it will be super easy with Maven and Quarkus framework.
But we should have some preparation on AWS before deploy and run our application.
...
of AWS for us, and we can deploy it by sam cli now.
We should use JVM one this time, and I would like to add some small modification on it.
...
## What solution do we have for the cold-start?
There are two approaches for this main issue.

+ Using Provisioned Concurrency
  It's not scope of this article and please visit [Predictable start-up times with Provisioned Concurrency](https://aws.amazon.com/blogs/compute/new-for-aws-lambda-predictable-start-up-times-with-provisioned-concurrency/)
+ Having better performance on initialize time and response time of the Java application.
  The main question is how do we can make better performance in our Java application? My answer is make a native binary executable from our Java application and deploy it on AWS Lambda.
  and [Oracle GraalVM](https://graalvm.org) makes it possible.
...
languages.
Native Image is a technology to ahead-of-time compile Java code to a standalone executable, called a native image. This
executable includes the application classes, classes from its dependencies, runtime library classes, and statically
linked native code from JDK. It does not run on the Java VM, but includes necessary components like memory management,
thread scheduling, and so on from a different runtime system, called “Substrate VM”.

## Build a native binary executable from the Java application
We need to install GraalVM, and it's Native-Image from [this guideline](https://thinksky.com/blog/2020/11/03/install-graalvm-and-native-image/) first. 
After install GraalVM, we can convert a Java application to native binary executable with GraalVM. 
Quarkus makes it easy for us, and it has a maven/gradle plugin for it, so in a typical Quarkus base application we will have a profile called native

```shell
$  mvn clean install -Pnative
```

Maven will build a native binary executable file, and it will be executable base on your local machine, if you are on Windows, this file will be only runnable on Windows machine.
But AWS Lambda will deploy on AWS Linux(v1 or v2), this binary file should runnable on Linux. In this case Quarkus framework will cover this requirement by a simple parameter on its plugin.
...
```
As mention on the command _**-Dquarkus.native.builder-image**_, we can specify what version of GraalVm we want to use to create a native binary file.
...
- point: **provided** will use Amazon Linux and **provided.al2** will use  [Amazon Linux 2](https://aws.amazon.com/amazon-linux-2/faqs/), so because of long-term support of version 2, I would like suggest use version 2.

### Deploy built binary executable on AWS Lambda

As we saw, Quarkus will produce two sam templates for us, one is for JVM base Lambda and second one is the native binary executable.
We should use native one this time, and I would like to have some small modification on it as well.
...
```
This command will upload binary file as a zip format to AWS and deploy it as Lambda Function exactly like JVM version, and we can jump to interesting part monitoring performance.
...
We can analyze and compare both version of application on AWS Lambda platform, and it can be comparable in two categories.
+ **Initialize time**
  The first call or invoke of the Lambda function is Initialize time, and it will be almost the longest duration of invoking an application,
  because our Java application will be start from scratch in this phase.

  ![](images/initialize-time.png)
  It is supper big difference between JVM and Binary version, and it means initial time of native binary version almost is **_8 times_** faster than JVM version.

<br/>

+ **Invoke times**
  TBD:~~When the application initialized, I invoke them seven time and~~ 
